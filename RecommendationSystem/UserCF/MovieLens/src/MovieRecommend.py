...
def PrecisionRecall(train, test, W, N, K):  # N为推荐物品的TopN，K为和用户兴趣相似的K个用户
    ...
    for user in train.keys():
        ...
        rank_all = UserCF.UserCF_IIFRecommend(user, train, W, K)    # 给训练集中的user做的和他兴趣相似的所有用户有过行为的全部物品的推荐
...
def main():
    ...
    data = [tuple(line.split('::')[:2]) for line in open('G:/master/python/PycharmProjects/RecommendationSystem/UserCF/MovieLens/data/ml-1m/ratings.dat').readlines()]  # win10上的ml-1m数据集
